...
def inbag_times_(self, X):
    ...
    for t_idx in range(self.n_estimators):
        sample_idx = _generate_sample_indices(
            self.estimators_[t_idx].random_state, n_samples)
...
def cls(self, X, y):
    ...
    for index, tree in enumerate(self.estimators_):
        ...
        weighted_n_node_samples = np.array(np.sum(decision_path_inb, axis=0))[
            0] / tree_X_inb.shape[0]

        for i in range(n_nodes):

            arr1 = tree_y_oob[np.array(decision_path_oob[:, i]).ravel().nonzero()[
                0].tolist()]
            arr2 = tree_y_inb[np.array(decision_path_inb[:, i]).ravel().nonzero()[
                0].tolist()]

            if len(arr1) == 0 or len(arr2) == 0:
                if sum(tree.tree_.children_left == i) > 0:
                    parent_node = np.arange(
                        n_nodes)[tree.tree_.children_left == i][0]
                    flag[parent_node] = False
                elif sum(tree.tree_.children_right == i) > 0:
                    parent_node = np.arange(
                        n_nodes)[tree.tree_.children_right == i][0]
                    flag[parent_node] = False
                # Если это лист или корень без родителя, просто пропускаем
                impurity[i] = 0
            else:
